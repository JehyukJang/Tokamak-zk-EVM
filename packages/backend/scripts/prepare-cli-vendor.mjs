...
function usage() {
  throw new Error('Usage: node packages/backend/scripts/prepare-cli-vendor.mjs --output <packages/cli/vendor/backend>');
}
...
async function makeRuntimeOnlyWorkspace(output) {
  ...
  await rewriteFile(path.join(output, 'libs', 'Cargo.toml'), [
    ['\n[[bench]]\nname = "outer_product_bench"\nharness = false\n', '\n'],
    ['\n[[bench]]\nname = "matrix_matrix_mul_bench"\nharness = false\n', '\n'],
    ['\ncriterion = "0.3"\n', '\n'],
  ]);
  await rewriteFile(path.join(output, 'prove', 'Cargo.toml'), [[
  ...
  await rewriteFile(path.join(output, 'build-support', 'subcircuit_library.rs'), [
  ...
  await rewriteFile(path.join(output, 'libs', 'src', 'lib.rs'), [[
    '#[path = "../../../../versioning/compatibility.rs"]',
    '#[path = "../../versioning/compatibility.rs"]',
  ]]);
  await rewriteFile(path.join(output, 'contracts', 'rust', 'backend_build_metadata.rs'), [[
    '#[path = "../../../../versioning/compatibility.rs"]',
    '#[path = "../../versioning/compatibility.rs"]',
  ]]);
}
