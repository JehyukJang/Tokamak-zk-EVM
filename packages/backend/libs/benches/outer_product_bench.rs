...
use libs::vector_operations::outer_product_two_vecs;
...
fn bench_outer_products(c: &mut Criterion) {
    ...
    let mut result: Box<[ScalarField]> = vec![ScalarField::zero(); total].into_boxed_slice();

    c.bench_function("outer_product_two_vecs", |b| {
        b.iter(|| {
            outer_product_two_vecs(&col_vec, &row_vec, &mut result);
        })
    });
}
