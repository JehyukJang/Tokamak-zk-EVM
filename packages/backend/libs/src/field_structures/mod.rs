...
impl PlacementVariables {
    pub fn gen_dummy(
        ...
        let dummy = Self {
            subcircuitId: 0,
            variables: vec![HexString('0'.to_string())].into_boxed_slice(),
        };
        let mut placement_variables_dummy: Box<[Self]> =
            vec![dummy; setup_params.s_max].into_boxed_slice();
        for i in 0..setup_params.s_max {
            ...
            let variables_hex: Vec<HexString> = variables_val
                .iter()
                .map(|x| HexString(x.to_string()))
                .collect();
