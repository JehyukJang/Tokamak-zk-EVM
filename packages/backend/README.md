...
library path. A release build without `local-development-subcircuit-library` embeds the npm
subcircuit-library snapshot and does not take `--subcircuit-library`. A build with
`local-development-subcircuit-library` requires `--subcircuit-library <PATH>` even when it uses
Cargo's release profile. The path must point to a generated local subcircuit-library directory,
typically:
...
explicit runtime path; without that feature, a release MPC build prepares the npm
subcircuit-library snapshot. The build records the selected input origin in final CRS provenance,
and the MPC setup flow still consumes only `r1cs/subcircuit*.r1cs` from it.
