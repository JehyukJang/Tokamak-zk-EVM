...
try {
  ...
  for(const test of fixtures.cases) {
    const setup: SetupParams = { l_user: 0, l_user_out: 0, ...test.setup };
    ...
    assert.equal(arithmeticIndex(domain, setup, setup.s_max - 1, 0, setup.n - 1), e.arithmeticIndex);
    assert.equal(connectionIndex(setup, setup.s_max - 1, setup.l_D - setup.l - 1), e.connectionIndex);
  ...
  const setup: SetupParams = { l_free: 2, l: 2, l_D: 4, l_user: 0, l_user_out: 0, m: 2, m_D: 6, t: 4, n: 2, s_D: 3, s_max: 2 };
  ...
  assert.throws(() => deriveUnivariateDomainShape(f, { ...setup, t: 8 }));
}
finally {
