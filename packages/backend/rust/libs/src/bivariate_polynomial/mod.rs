...
extern "C" {
    ...
    fn bls12_381_poly_eval(
        coeffs: *const ScalarField,
        coeffs_size: u64,
        domain: *const ScalarField,
        domain_size: u64,
        config: *const VecOpsConfig,
        evals: *mut ScalarField,
    ) -> eIcicleError;
}

pub(crate) fn polynomial_eval_batch<
    C: HostOrDeviceSlice<ScalarField> + ?Sized,
    D: HostOrDeviceSlice<ScalarField> + ?Sized,
    O: HostOrDeviceSlice<ScalarField> + ?Sized,
>(
    coeffs: &C,
    coeffs_size: usize,
    domain: &D,
    ...
    if coeffs_size == 0 || domain.is_empty() || batch_size == 0 {
        return Err(eIcicleError::InvalidArgument);
    }

    let expected_coeffs = coeffs_size
    ...
    let batch_size = i32::try_from(batch_size).map_err(|_| eIcicleError::InvalidArgument)?;
    let mut config = VecOpsConfig::default();
    config.is_a_on_device = coeffs.is_on_device();
    config.is_b_on_device = domain.is_on_device();
    config.is_result_on_device = evals.is_on_device();
    ...
    Self::FieldConfig: FieldConfig,
{
    ...
    fn zero() -> Self;
    fn from_coeffs<S: HostOrDeviceSlice<Self::Field> + ?Sized>(
        coeffs: &S,
        x_size: usize,
        y_size: usize,
    ) -> Self;
    ...
    fn div_by_ruffini(&self, x: &Self::Field, y: &Self::Field) -> (Self, Self, Self::Field)
    where
        Self: Sized;
    ...
    fn copy_coeffs<S: HostOrDeviceSlice<Self::Field> + ?Sized>(
        &self,
        start_idx: u64,
        coeffs: &mut S,
    );
    ...
    fn divide_x(&self, denominator: &Self) -> (Self, Self)
    where
        Self: Sized;

    // Method to divide this polynomial by another, returning quotient and remainder.
    fn divide_y(&self, denominator: &Self) -> (Self, Self)
    where
        Self: Sized;

    fn _divide_uni(&self, denom: &Self, y_dir: bool) -> (Self, Self)
    where
        Self: Sized;
    ...
    fn _div_uni_coeffs_by_ruffini(
        poly_coeffs_vec: &[Self::Field],
        x: &Self::Field,
    ) -> (Vec<Self::Field>, Self::Field);
}
...
impl BivariatePolynomial for DensePolynomialExt {
        ...
        Out: HostOrDeviceSlice<Self::Field> + ?Sized,
    {
        ...
        {
            // Transform along Y as row batches, then along X as column batches.
    ...
    fn scale_coeffs_x(&self, x_factor: &Self::Field) -> Self {
        ...
        return DensePolynomialExt::from_coeffs(scaled_coeffs, self.x_size, self.y_size);
    }
    ...
    fn _scale_coeffs<S: HostOrDeviceSlice<Self::Field> + ?Sized>(
        ...
        if !y_dir {
            ...
            for ind in 0..x_size {
                left_scale[ind * y_size..(ind + 1) * y_size]
                    .copy_from_host(HostSlice::from_slice(&vec![scaler; y_size]))
                    .unwrap();
        ...
        if y_dir {
            ...
            let mut scaler = Self::Field::one();
            for ind in 0..y_size {
                _right_scale[ind * x_size..(ind + 1) * x_size]
                    .copy_from_host(HostSlice::from_slice(&vec![scaler; x_size]))
                    .unwrap();
                scaler = scaler.mul(*factor);
            }
    ...
    fn _slice_coeffs_into_blocks(
        ...
        for row_idx in 0..self.x_size {
            ...
            for col_idx in 0..self.y_size {
                ...
                let in_block_idx =
                    block_y_size * (row_idx % block_x_size) + (col_idx % block_y_size);
    ...
    fn eval_three_batch(
        ...
        let coefficient_count = self
            .x_size
            .checked_mul(self.y_size)
            .ok_or(eIcicleError::InvalidArgument)?;
        let mut coefficients = DeviceVec::<Self::Field>::device_malloc(coefficient_count)?;
        self.copy_coeffs(0, &mut coefficients);

        let y_domain_host = [*y, *shifted_y];
        let mut y_domain = DeviceVec::<Self::Field>::device_malloc(y_domain_host.len())?;
        y_domain.copy_from_host(HostSlice::from_slice(&y_domain_host))?;
        let first_stage_len = self
            .x_size
            .checked_mul(y_domain_host.len())
            .ok_or(eIcicleError::InvalidArgument)?;
        let mut first_stage = DeviceVec::<Self::Field>::device_malloc(first_stage_len)?;
    ...
    fn get_univariate_polynomial_x(&self, idx_y: u64) -> Self {
        Self {
            poly: self
                .poly
                .slice(idx_y, self.y_size as u64, self.x_size as u64),
    ...
    fn get_univariate_polynomial_y(&self, idx_x: u64) -> Self {
        Self {
            poly: self
                .poly
                .slice(idx_x * self.y_size as u64, 1, self.y_size as u64),
            x_size: 1,
    ...
    fn resize(&mut self, target_x_size: usize, target_y_size: usize) {
        ...
        for i in 0..cmp::min(self.x_size, new_x_size) {
            ...
            res_coeffs_vec[new_y_size * i..new_y_size * i + each_y_size]
                .copy_from_slice(&orig_coeffs_vec[self.y_size * i..self.y_size * i + each_y_size]);
        }
    ...
    fn mul_monomial(&self, x_exponent: usize, y_exponent: usize) -> Self {
        if x_exponent == 0 && y_exponent == 0 {
            ...
            for i in 0..self.x_size {
                res_coeffs_vec[new_y_size * (i + x_exponent) + y_exponent
                    ..new_y_size * (i + x_exponent) + self.y_size + y_exponent]
                    .copy_from_slice(&orig_coeffs_vec[self.y_size * i..self.y_size * (i + 1)]);
            }
    ...
    fn _divide_uni(&self, denominator: &Self, y_dir: bool) -> (Self, Self)
    where
        Self: Sized,
    {
        // Division along Y (denom is assumed to be a polynomial of Y)
        let quo_size = if y_dir { self.y_size } else { self.x_size };
        let rem_size = quo_size;
        let sweep_dir = if y_dir { self.x_size } else { self.y_size };
        ...
        for offset in 0..sweep_dir {
            ...
            quo_coeffs_vec[offset * quo_size..(offset + 1) * quo_size]
                .copy_from_slice(&sub_quo_coeffs_vec);
            rem_coeffs_vec[offset * rem_size..(offset + 1) * rem_size]
                .copy_from_slice(&sub_rem_coeffs_vec);
        }
    ...
    fn div_by_vanishing(
        ...
        let find_cache_index =
            |cache: &Box<[DenomCache]>, x_size: usize, y_size: usize, base: usize| {
                cache.iter().position(|entry| {
                    entry.x_size == x_size && entry.y_size == y_size && entry.base == base
                })
            };
        ...
        let quo_y = {
            let mut quo_y_tilde = DeviceVec::<Self::Field>::device_malloc(c * n * d).unwrap();
            {
                let denom_y_eval_inv_slice = if let Some(idx) = cached_denom_y_index {
                    ...
                    let evals = build_denom_inv(c, n * d, d, None, Some(&xi));
        ...
        let quo_x = {
            ...
            let mut quo_x_tilde = DeviceVec::<Self::Field>::device_malloc(m * c * n * d).unwrap();
            {
                ...
                let denom_x_eval_inv_slice = if let Some(idx) = cached_denom_x_index {
                    ...
                    let evals = build_denom_inv(m * c, n * d, c, Some(&zeta), None);
                ...
                Self::FieldConfig::mul(&b_tilde, denom_x_eval_inv, &mut quo_x_tilde, &vec_ops_cfg)
                    .unwrap();
            }
    ...
    fn div_by_vanishing_opt(&mut self, denom_x_degree: i64, denom_y_degree: i64) -> (Self, Self) {
        ...
        let x_size = m * c;
        let y_size = n * d;
        let mut p_coeffs_vec = vec![Self::Field::zero(); x_size * y_size];
        self.copy_coeffs(0, HostSlice::from_mut_slice(&mut p_coeffs_vec));

        let mut acc_block_vec = vec![Self::Field::zero(); c * y_size];
        for block_x in 0..m {
            let x_offset = block_x * c;
            for local_x in 0..c {
                let src_start = (x_offset + local_x) * y_size;
                let dst_start = local_x * y_size;
                for y in 0..y_size {
                    acc_block_vec[dst_start + y] =
                        acc_block_vec[dst_start + y] + p_coeffs_vec[src_start + y];
                }
            }
        }

        let mut quo_y_coeffs_vec = vec![Self::Field::zero(); c * y_size];
        if y_size > d {
            for x in 0..c {
                let row_start = x * y_size;
                for y in 0..(y_size - d) {
                    let prev = if y >= d {
                        quo_y_coeffs_vec[row_start + y - d]
                    } else {
                        Self::Field::zero()
                    };
                    quo_y_coeffs_vec[row_start + y] = prev - acc_block_vec[row_start + y];
                }
            }
        }

        let mut b_coeffs_vec = p_coeffs_vec;
        if y_size > d {
            for x in 0..c {
                let row_start = x * y_size;
                for y in 0..(y_size - d) {
                    let coeff = quo_y_coeffs_vec[row_start + y];
                    b_coeffs_vec[row_start + y] = b_coeffs_vec[row_start + y] + coeff;
                    b_coeffs_vec[row_start + y + d] = b_coeffs_vec[row_start + y + d] - coeff;
                }
            }
        }

        let mut quo_x_coeffs_vec = vec![Self::Field::zero(); x_size * y_size];
        if x_size > c {
            for x in 0..(x_size - c) {
                let row_start = x * y_size;
                for y in 0..y_size {
                    let prev = if x >= c {
                        quo_x_coeffs_vec[(x - c) * y_size + y]
                    } else {
                        Self::Field::zero()
                    };
                    quo_x_coeffs_vec[row_start + y] = prev - b_coeffs_vec[row_start + y];
                }
            }
        }
        ...
        #[cfg(debug_assertions)]
        {
            if y_size > d {
                for x in 0..c {
                    let row_start = x * y_size;
                    for y in (y_size - d)..y_size {
                        debug_assert_eq!(
                            acc_block_vec[row_start + y],
                            quo_y_coeffs_vec[row_start + y - d]
                        );
                    }
                }
            ...
            if x_size > c {
                for x in (x_size - c)..x_size {
                    for y in 0..y_size {
                        debug_assert_eq!(
                            b_coeffs_vec[x * y_size + y],
                            quo_x_coeffs_vec[(x - c) * y_size + y]
                        );
                    }
                }
        ...
        let mut quo_x = DensePolynomialExt::from_coeffs(
            HostSlice::from_slice(&quo_x_coeffs_vec),
            x_size,
            y_size,
        );
        let mut quo_y =
            DensePolynomialExt::from_coeffs(HostSlice::from_slice(&quo_y_coeffs_vec), c, y_size);
        if x_size > c {
            quo_x.x_degree = (x_size - c - 1) as i64;
            quo_x.y_degree = y_size as i64 - 1;
        } else {
            quo_x.x_degree = -1;
            quo_x.y_degree = -1;
        }
        if y_size > d {
            quo_y.x_degree = c as i64 - 1;
            quo_y.y_degree = (y_size - d - 1) as i64;
        } else {
            quo_y.x_degree = -1;
            quo_y.y_degree = -1;
        }
        ...
        Self: Sized,
    {
        ...
        let mut q_x_coeffs_vec_flat: Vec<Self::Field> =
            q_x_coeffs_vec.into_par_iter().flatten().collect();
        transpose_inplace(&mut q_x_coeffs_vec_flat, y_len, x_len);
        let q_x = DensePolynomialExt::from_coeffs(
            HostSlice::from_slice(&q_x_coeffs_vec_flat),
            x_len,
            y_len,
        );

        // Divide R_X(Y) by (Y-y).
        let (q_y_coeffs_vec, r_y) =
