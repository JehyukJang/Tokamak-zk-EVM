import { BIGINT_0, BIGINT_1, bigIntToBytes, bytesToBigInt, bytesToHex, setLengthLeft, } from "@ethereumjs/util/index.js";
import { keccak256 } from 'ethereum-cryptography/keccak.js';
import { EOFBYTES, isEOF } from '../../eof/util.js';
import { createAddressFromStackBigInt, getDataSlice } from '../../opcodes/util.js';
import { DEFAULT_SOURCE_SIZE, INITIAL_PLACEMENT_INDEX, KECCAK_IN_PLACEMENT, KECCAK_IN_PLACEMENT_INDEX, KECCAK_OUT_PLACEMENT, KECCAK_OUT_PLACEMENT_INDEX, LOAD_PLACEMENT, LOAD_PLACEMENT_INDEX, RETURN_PLACEMENT, RETURN_PLACEMENT_INDEX, } from '../constant/index.js';
...
import { InvalidInputCountError, SynthesizerError, SynthesizerValidator, } from '../validation/index.js';
export const synthesizerArith = (op, ins, out, runState) => {
...
export const synthesizerBlkInf = (op, runState, target) => {
    let dataPt;
    switch (op) {
        case 'BLOCKHASH':
        case 'BLOBHASH':
        ...
        case 'BLOBBASEFEE':
            ...
            dataPt = runState.synthesizer.loadBlkInf(runState.env.block.header.number, op, runState.stack.peek(1)[0]);
            break;
        default:
...
export async function prepareEXTCodePt(runState, target, _offset, _size) {
    ...
    const dataPt = runState.synthesizer.loadEnvInf(address.toString(), codeType, dataBigint, codeOffsetNum, Number(dataLength));
    return dataPt;
}
export async function synthesizerEnvInf(op, runState, target, offset) {
    ...
    switch (op) {
        case 'CALLDATALOAD': {
            ...
            if (calldataMemoryPts.length > 0) {
                ...
                if (dataAliasInfos.length > 0) {
                    ...
                    dataPt = runState.synthesizer.loadEnvInf(runState.env.address.toString(), 'Calldata(Empty)', runState.stack.peek(1)[0], i);
                }
            }
            else {
                // Case: The calldata is originated from the user (transciton) input
                dataPt = runState.synthesizer.loadEnvInf(runState.env.address.toString(), 'Calldata(User)', runState.stack.peek(1)[0], i);
            }
            break;
        }
        case 'BALANCE':
        ...
        case 'RETURNDATASIZE':
            // These opcodes have no input and one output
            dataPt = runState.synthesizer.loadEnvInf(runState.env.address.toString(), op, runState.stack.peek(1)[0]);
            break;
        default:
...
export class Synthesizer {
    ...
    constructor() {
        ...
        if (subcircuits[1].name === LOAD_PLACEMENT.name &&
     ...
     * @private
     */
    _addWireToLoadPlacement(pointerIn) {
        // Use the length of existing output list as index for new output
        if (this.placements.get(LOAD_PLACEMENT_INDEX).inPts.length !==
     ...
     * @returns {void}
     */
    loadPUSH(codeAddress, programCounter, value, size) {
        const inPtRaw = {
    ...
    loadEnvInf(codeAddress, type, value, _offset, size) {
    ...
    loadStorage(codeAddress, key, value) {
    ...
    storeStorage(codeAddress, key, inPt) {
        const keyString = JSON.stringify({ address: codeAddress, key: Number(key) });
        this.storagePt.set(keyString, inPt);
    ...
    loadBlkInf(blkNumber, type, value) {
        const whereItFrom = {
    ...
    loadKeccak(inPts, outValue, length) {
        // 연산 실행
    ...
    placeMSTORE(dataPt, truncSize) {
        ...
        if (truncSize < dataPt.sourceSize) {
            ...
            if (dataPt.value !== outValue) {
                ...
                const inPts = [this.loadAuxin(BigInt(maskerString)), dataPt];
                const rawOutPt = {
                ...
                const outPts = [DataPointFactory.create(rawOutPt)];
    ...
    executeOperation(name, values) {
        const operation = OPERATION_MAPPING[name];
        return operation(...values);
    }
    createOutputPoint(value, _wireIndex) {
    ...
    adjustMemoryPts = (dataPts, memoryPts, srcOffset, dstOffset, viewLength) => {
     ...
     * @returns {DataPt} Generated data point.
     */
    _resolveDataAlias(dataAliasInfos) {
        const ADDTargets = [];
    ...
    _applyShift(info) {
        ...
        if (Math.abs(shift) > 0) {
            ...
            const subcircuitName = shift > 0 ? 'SHL' : 'SHR';
            const absShift = Math.abs(shift);
            const inPts = [this.loadAuxin(BigInt(absShift)), dataPt];
     ...
     */
    _applyMask(info, unshift) {
        let masker = info.masker;
        const { shift, dataPt } = info;
        if (unshift === true) {
            const maskerBigint = BigInt(masker);
            const unshiftMaskerBigint = shift > 0
        ...
        if (maskOutValue !== dataPt.value) {
            const inPts = [this.loadAuxin(BigInt(masker)), dataPt];
