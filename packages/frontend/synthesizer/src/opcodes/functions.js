...
export const handlers = new Map([
            ...
            if (dataLength !== BIGINT_0) {
                ...
                if (calldataMemoryPts.length > 0) {
                    ...
                    const entryToCopy = {
                        ...
                        dataPt: runState.synthesizer.loadEnvInf(runState.env.address.toString(), 'Calldata', bytesToBigInt(data), dataOffsetNum, Number(dataLength)),
                    };
    ...
    [
        ...
        function (runState) {
            ...
            if (dataLength !== BIGINT_0) {
                ...
                const dataPt = runState.synthesizer.loadEnvInf(runState.env.address.toString(), 'Code', dataBigint, codeOffsetNum, Number(dataLength));
    ...
    [
        ...
        function (runState) {
            runState.stack.push(BIGINT_0);
            // For Synthesizer //
            const dataPt = runState.synthesizer.loadPUSH(runState.env.address.toString(), runState.programCounterPrev, BIGINT_0, 1);
            runState.stackPt.push(dataPt);
            if (runState.stackPt.peek(1)[0].value !== runState.stack.peek(1)[0]) {
    ...
    [
        ...
        async function (runState, common) {
            ...
            runState.messageGasLimit = undefined;
            // For synthesizer
            const [__currentGasLimit, _toAddr, _value, _inOffset, _inLength, _outOffset, _outLength] = runState.stackPt.popN(7);
    ...
    [
        ...
        async function (runState) {
            ...
            runState.messageGasLimit = undefined;
            // For synthesizer
            const [__currentGasLimit, _toAddr, _inOffset, _inLength, _outOffset, _outLength] = runState.stackPt.popN(6);
            if (__currentGasLimit.value !== _currentGasLimit ||
                _toAddr.value !== toAddr ||
                _inOffset.value !== inOffset ||
                _inLength.value !== inLength ||
